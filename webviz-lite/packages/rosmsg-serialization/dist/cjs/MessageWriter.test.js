...
describe("MessageWriter", () => {
    describe("simple type", () => {
        const testNum = (type, size, expected, cb) => {
            ...
            it(`writes message ${JSON.stringify(message)} containing ${type}`, () => {
                const writer = new MessageWriter_1.MessageWriter((0, rosmsg_1.parse)(`${type} foo`));
        ...
        it("writes strings", () => {
            const writer = new MessageWriter_1.MessageWriter((0, rosmsg_1.parse)("string name"));
            const buff = getStringBytes("test");
            expect(writer.writeMessage({ name: "test" })).toEqual(buff);
        });
        it("writes time", () => {
            const writer = new MessageWriter_1.MessageWriter((0, rosmsg_1.parse)("time right_now"));
    ...
    describe("array", () => {
        ...
        describe("typed arrays", () => {
            ...
            it("writes a uint8[] with a fixed length", () => {
                const writer = new MessageWriter_1.MessageWriter((0, rosmsg_1.parse)("uint8[3] values\nuint8 after"));
                const message = { values: Uint8Array.from([1, 2, 3]), after: 4 };
                const result = writer.writeMessage(message);
    ...
    describe("complex types", () => {
        it("writes single complex type", () => {
            const writer = new MessageWriter_1.MessageWriter((0, rosmsg_1.parse)("string firstName \n string lastName\nuint16 age"));
            const buff = concat([
                getStringBytes("foo"),
                getStringBytes("bar"),
      ...
      uint16 id
      `;
            const writer = new MessageWriter_1.MessageWriter((0, rosmsg_1.parse)(messageDefinition));
            const buff = concat([
                getStringBytes("foo"),
                getStringBytes("bar"),
      ...
      `;
            ...
            const buff = concat([
                ...
                new Uint8Array([100, 0x00]),
                new Uint8Array([0x03, 0x00, 0x00, 0x00]),
                getStringBytes("http://foo.com"),
                new Uint8Array([10]),
                getStringBytes("http://bar.com"),
                new Uint8Array([12]),
                getStringBytes("http://zug.com"),
                new Uint8Array([16]),
                getStringBytes("baz"),
                new Uint8Array([101, 0x00]),
                new Uint8Array([0x00, 0x00, 0x00, 0x00]),
      ...
      string name # a description of the test/component reporting`;
        it("writes bytes and constants", () => {
            const writer = new MessageWriter_1.MessageWriter((0, rosmsg_1.parse)(withBytesAndBools));
