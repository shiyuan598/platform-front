...
function Event({ event, index }: { event: ZhitoEvent, index: number }): JSX.Element {
      ...
      <Stack style={{ paddingTop: "5px", width: "90%" }} >
        ...
        <Stack direction="row" spacing={1} justifyContent="space-between">
          ...
          <Typography variant="caption" display="block" className={classes.descriptionInput}
            ...
            onBlur={() => { setFocus(false); updateEvent(event.timestamp, inputText) }}
...
function EventList({events,}: {events:ZhitoEvent[]}): JSX.Element {
  // const { events } = useRecordStore(state => state)
  return <Stack divider={<Divider orientation="horizontal" flexItem />}>
    {events.map((event, i) => <Event event={event} index={events.length - i -1} key={events.length - i -1} />)}
  </Stack>
}
...
function PanelList(_props: Props): JSX.Element {
  const classes = useStyles();
    ...
    <div className={classes.root}>
      <div className={classes.sticky}>
        <Stack className={classes.container} spacing={2}>
          ...
          <EventList events={recordMap[selected]?.events||[]}/>
