...
import { EventRecorderSidebar } from "@foxglove/studio-base/components/EventRecorderSidebar";
...
import HelpSidebar, { MESSAGE_PATH_SYNTAX_HELP_INFO } from "@foxglove/studio-base/components/HelpSidebar";
...
import RemoteControlSidebar from "@foxglove/studio-base/components/RemoteControlSidebar";
    ...
    >
      {selectedLayoutId == undefined ? (
        <Text styles={{ root: { color: theme.palette.neutralTertiary } }}>
          请先<Link onClick={openLayoutBrowser}>添加布局</Link>
        </Text>
      ) : (
...
export default function Workspace(props: WorkspaceProps): JSX.Element {
  ...
  const openHandle = useCallback(
    async (handle: FileSystemFileHandle) => {
      ...
      const matchedSource = availableSources.find((source) => {
        ...
        return source.supportedFileTypes?.includes(ext) || (source.supportedFileTypes?.[0]===".00000"&&ext.startsWith(".0"));
      });
      if (matchedSource) {
  ...
  const sidebarItems = useMemo<Map<SidebarItemKey, SidebarItem>>(() => {
    const SIDEBAR_ITEMS = new Map<SidebarItemKey, SidebarItem>([
      ...
      ["layouts", { iconName: "FiveTileGrid", title: "布局", component: LayoutBrowser }],
      ["add-panel", { iconName: "RectangularClipping", title: "添加面板", component: AddPanel }],
      [
        "panel-settings",
        { iconName: "SingleColumnEdit", title: "设置面板", component: PanelSettings },
      ],
      [
        "event-recorder",
        { iconName: "NumberedList", title: "事件记录", component: EventRecorderSidebar },
      ],
      [
        "remote-control",
        { iconName: "Control", title: "远程控制", component: RemoteControlSidebar },
      ],
    ...
    return supportsAccountSettings
      ...
      : SIDEBAR_ITEMS;
  }, [supportsAccountSettings, playerProblems]);

  const sidebarBottomItems: readonly SidebarItemKey[] = useMemo(() => {
    return supportsAccountSettings ? ["help"] : ["help"];
