 ...
 * @param precision precision in nsec
 * @returns Equality result
 */
 export function areEqual(left: Time, right: Time, precision?:number): boolean {
  if(precision!==undefined){
    return Math.abs((left.sec - right.sec) * 1e9 + (left.nsec - right.nsec)) < precision
  } else{
...
function getSynchronizedMessages(
  ...
  for (const topic of topics) {
    const matchingMessage = messages[topic]?.find(({ message }) => {
      ...
      return thisStamp && areEqual(stamp, thisStamp,3e7);
    });
    if (!matchingMessage) {
      return undefined;
    }
