...
export default function App(props: AppProps): JSX.Element {
  ...
  const [_globalLaunchPreference = "unknown"] = useAppConfigurationValue<string>(
    AppSetting.LAUNCH_PREFERENCE,
  );
  const [_sessionLaunchPreference] = useSessionStorageValue(AppSetting.LAUNCH_PREFERENCE);

  // Session preferences take priority over global preferences.
  // const _activePreference = "web"//sessionLaunchPreference ?? globalLaunchPreference;
  if (isDesktop) {
    return <AppContent {...props} />;
  } else {
    // const url = new URL(window.location.href);
    // const _hasParams = Array.from(url.searchParams.entries()).length > 0;
