...
export function getMarkerOptions(
  ...
  imageMarkerPrototypes:string[]
): string[] {
  const results = [];
  // const cameraNamespace = getCameraNamespace(imageTopic);
  for (const { name, datatype,proto } of topics) {
    if (
      imageMarkerDatatypes.includes(datatype)||imageMarkerPrototypes.includes(proto??"")
    ) {
      results.push(name);
    }
  }
...
export function buildMarkerData(rawMarkerData: RawMarkerData): MarkerData | undefined {
  const { markers, cameraInfo } = rawMarkerData;
  if (markers.length === 0) {
    return {
      markers,
