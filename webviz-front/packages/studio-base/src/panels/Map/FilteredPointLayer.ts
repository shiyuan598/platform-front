...
// import { NavSatFixMsg, ZhitoPoseMsg, NavSatFixPositionCovarianceType } from "./types";
import { ZhitoPoseMsg } from "./types";
...
type Args = {
  ...
  navSatMessageEvents: readonly MessageEvent<ZhitoPoseMsg>[];
  onHover?: (event: MessageEvent<ZhitoPoseMsg> | undefined) => void;
  onClick?: (event: MessageEvent<ZhitoPoseMsg>) => void;
};

class PointMarker extends CircleMarker {
  messageEvent?: MessageEvent<ZhitoPoseMsg>;
}
let lastViewStamp = Date.now();
/**
 * Create a leaflet LayerGroup with filtered points
 */
function FilteredPointLayer(args: Args): FeatureGroup {
  const { navSatMessageEvents: points, map } = args;//bounds,
  ...
  for (const messageEvent of points) {
    ...
    if (points.length === 1) {
      const now = Date.now();
      if ((now - lastViewStamp) > 100) {
        map.setView({ lng: lon, lat })
        lastViewStamp = now;
      }
    }
