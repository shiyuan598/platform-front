...
const Colors = [
  { r: 244 / 255, g: 67 / 255, b: 54 / 255, a: 0.7 },
  { r: 33 / 255, g: 150 / 255, b: 243 / 255, a: 0.7 },
  { r: 139 / 255, g: 195 / 255, b: 74 / 255, a: 0.7 },
  { r: 1.0, g: 193 / 255, b: 7 / 255, a: 0.7 }]
function TopicTree({
  allKeys,
  ...
  const dataSources = useDataSourceInfo();
  const handleColorByTopic = useCallback((_event: unknown) => {
    const val = !colorByTopic;
    setColorByTopic(val);
    const setting: Setting = {
      settingsByKey: {}
    }
    if (val) {
      dataSources.topics.filter(t => t.name.search("perception") > -1).forEach((topic, i) => {
        if (topic.datatype == "visualization_msgs/MarkerArray") {
          setting.settingsByKey["t:" + topic.name] = {
            overrideColor: Colors[i % 4]
          }
        }
      })
    }
    saveConfig({
      settingsByKey: {
        ...setting.settingsByKey,
      },
    })
  }, [colorByTopic, saveConfig, dataSources])
  return (
    <Stack className={classes.inner} styles={{ root: { width: treeWidth } }}>
      ...
      <Stack horizontal grow styles={{ root: { position: "relative" } }}>
        <Checkbox label="分通道着色" onChange={handleColorByTopic} checked={colorByTopic} />
      </Stack>
    </Stack>
  );
}
