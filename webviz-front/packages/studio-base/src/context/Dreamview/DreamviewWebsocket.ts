...
export default class RealtimeWebSocketEndpoint extends Evented {
    ...
    simWorldUpdatePeriodMs: number;
    simWorldLastUpdateTimestamp: number;
    mapUpdatePeriodMs: number;
    mapLastUpdateTimestamp: number;
    updatePOI: boolean;
    routingTime: undefined;
    currentMode: null;
    worker: any;
    secondLastSeqNum: any;
    ready: boolean;
    ...
    requestRoute(start: { x: number, y: number, heading: number | null }, start_heading: number | null, waypoint: [number, number][], end: [number, number], parkingInfo: [number, number][], current_poi: string) {
        if (!this.ready) {
            return
        }
        const request = {
    ...
    requestMapElementIdsByRadius(radius: number) {
        if (!this.ready) {
            return
        }
        this.websocket?.send(JSON.stringify({
